...
- Evidence reviewed:
  ...
  - `.omx/specs/deep-interview-banner-management.md`: clarified admin-managed home banner requirements, including independent banner images, internal links, immediate publish/delete behavior, ordering, font/color controls, and explicit non-goals.
  ...
  - `src/components/buy-box.tsx`, `src/components/product-detail-sections-renderer.tsx`, and live review of `/products/hydration-skincare-set`: product detail purchase CTA visibility, heavy-border density, long-scroll behavior, and review/detail sequencing.
...
- Core routes/screens: Home, Shop, Product Detail, Auth, Checkout, Checkout Success, Account, Orders, Promoter, Policies, Admin, Admin Product List, Admin Product Editor, Admin Banner Management, Admin Orders, Admin Brand Partner Management, Brand Product List, Brand Product Detail Editor, Brand Reports.
- Content hierarchy: Storefront discovery uses expressive product imagery plus category/product-type visual cues; commerce/account/admin flows prioritize task headings, fields, status, policy links, and primary CTA. Account/My Page must read as a management hub first: order activity, default shipping readiness, profile identity, email updates, and help links are visible before any edit fields. The orders page is a customer order-management list with search, status filtering, sorting, and detail access. Order detail is an aftercare page, not a receipt dump: lead with `Order status`, status-specific copy, tracking availability, ordered date, and total, then show delivery progress, tracking details, items, payment, shipping address, and support. The About page is buyer-facing: lead with Korean beauty routine value, then curation, use guidance, purchase confidence, shipping/returns, and a shop CTA. Product detail pages must keep core commerce facts visible: hero/intro media, gallery, included items, routine steps, and legacy media/content blocks should not disappear just because a newer story document exists. Long product detail scrolls should keep purchase recovery available through a compact bottom CTA only after the primary buy box leaves the viewport; it should disappear again when the top buy box returns, so the first viewport does not duplicate purchase controls. Home banner content should come from operator-managed banner records, not active-product order, and should preserve a playful storefront rhythm while letting admins control copy, required background imagery, optional side/product imagery, internal destination, font, color, and ordering. Admin product editing must distinguish the customer-facing product brand label from the operational brand partner portal connection. Admin banner management should read as a compact merchandising queue: current banners, order controls, create/edit fields, image uploads, and immediate delete behavior without draft/scheduling chrome. Admin orders should keep search/filter/sort and order scanning as the primary surface; default order is newest-first, operators can switch sort modes, and fulfillment editing opens intentionally from an order card in a focused modal rather than occupying persistent page chrome. Brand partner management should prioritize lifecycle, members, health checks, assignment audit, and transfer/revocation. Brand detail editing should prioritize a document/story canvas over raw data fields; customer preview should be opened intentionally from the editor action area instead of occupying a persistent right pane.
...
- Long-scroll purchase recovery: Product detail pages should not force shoppers to scroll back to the hero after they become convinced. Use one compact, low-noise purchase action once the top buy box is off-screen.
...
- Admin banner simplicity: Banner management should stay intentionally smaller than product publishing. Create means visible, delete means removed, ordering is explicit, and controls should not imply hidden draft, schedule, analytics, targeting, or brand-side approval workflows.
...
- Shape/radius/elevation: 8px or smaller radius by default. Use borders, background contrast, spacing, and focus rings for hierarchy. Storefront product grids on Home, Make up, and Skincare should use square, rounded, cover-fit media frames with neutral 1px hairline borders; avoid thick black or category-color frames around product photos. Product detail pages should move away from thick black framing toward borderless or hairline surfaces so product imagery and purchase actions carry the visual weight. Pure media frames on product detail pages should usually be borderless with only radius/overflow, while purchase, quantity, thumbnail, floating CTA, and text-card surfaces can use visible neutral hairlines. Product detail photos should share rounded, stable frames; default to enlarged cover-fit for consistent visual size, and expose contain-fit when the full source image is more important than edge-to-edge fill. Keep 2px black borders out of product detail surfaces unless a control needs a deliberate active/focus affordance. Do not use right-bottom offset shadows anywhere in active product UI, including playful discovery, cards, buttons, badges, menus, modals, checkout, account, admin, brand, and form surfaces. Avoid authored `box-shadow`, Tailwind `shadow-*`, and arbitrary `shadow-[...]` utilities in source unless a future design decision explicitly documents a narrow exception.
